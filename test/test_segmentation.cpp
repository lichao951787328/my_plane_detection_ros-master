...
void initialRawPointsMat(size_t start_col_, size_t start_row_, size_t & width_, size_t & height_)
{
  cv::Mat depth_image = cv::imread("/home/humanoid/work/plane_detection/catkin_plane_detection_test_ws/src/plane_detection/bag/depth.png");
  std::cout<<"load image:"<<depth_image.cols<<" "<<depth_image.rows<<std::endl;
  width_ = depth_image.cols; height_ = depth_image.rows;
  orginazed_points raw_points_pcl;
  raw_points_pcl.initial(depth_image);
  raw_points = raw_points_pcl.Rect(start_col_, start_row_, width_, height_);
}
...
int main(int argc, char** argv)
{
  ...
  // pcl::PointCloud<pcl::PointXYZ> cloud;
  // getPointCloud(raw_points, cloud);
  ...
  // param.showParameter();
  quatree::node::setStaticMember(width, height, param.quatree_width, ROBOTWORLD_T_CAMERA);
  ...
  plane_segmentation ps(height, width, &qq);
